...
| `uv pip install -e .` fails with `torch==2.7.1` / `cp314` unsatisfiable | Your environment is using Python 3.14, but `torch==2.7.1` does not provide `cp314` wheels. Recreate your environment with Python 3.11/3.12/3.13 (for example: `conda create -n <env> -c conda-forge -y python=3.11`) and retry. |
| `uv sync` fails while building `av` on Jetson (`AVCodecParameters.ch_layout` missing) | This is usually a FFmpeg header/API mismatch (`av` expects newer FFmpeg than system 4.4 headers). Build/install FFmpeg 6+ locally and expose it to build tooling, e.g. `export PKG_CONFIG_PATH=/path/to/ffmpeg/lib/pkgconfig` and `export LD_LIBRARY_PATH=/path/to/ffmpeg/lib:$LD_LIBRARY_PATH`, then rerun `uv sync`. |
| `evdev==1.9.3` build fails (`KEY_LINK_PHONE` undeclared) on Jetson/Linux headers | Some kernel header versions do not provide this key code. Pin `evdev==1.9.2` via constraints and retry editable install: `printf 'evdev==1.9.2\n' > /tmp/uv-constraints.txt && GIT_LFS_SKIP_SMUDGE=1 uv pip install -e . -c /tmp/uv-constraints.txt`. |
