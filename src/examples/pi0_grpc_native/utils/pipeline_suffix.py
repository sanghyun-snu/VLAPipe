...
from .pipeline_logging import log_suffix_end
from .pipeline_logging import log_suffix_error
from .pipeline_logging import log_suffix_receive
from .pipeline_logging import log_suffix_received_done
from .pipeline_logging import log_suffix_start
...
class SuffixEvalSession:
    ...
    async def run(self) -> pb2.EvalResponse:
        ...
        try:
            receive_start_t = time.perf_counter()
            ...
            denoise_start_t = time.perf_counter()
            ...
            denoise_done_t = time.perf_counter()
            self._state.denoise_s = denoise_done_t - denoise_start_t
            self._state.total_s = time.perf_counter() - self._start_t
